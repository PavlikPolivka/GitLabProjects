...
public class GitLabCommentsListWorker {
  ...
  public static GitLabCommentsListWorker create(@NotNull final Project project, @NotNull final GitlabMergeRequest mergeRequest) {
    return GitLabUtil.computeValueInModal(project, "Loading comments...", (Convertor<ProgressIndicator, GitLabCommentsListWorker>) indicator -> {
      ...
      try {
        commentsListWorker.setComments(settingsState.api().getMergeRequestComments(mergeRequest));
      } catch (IOException e) {
